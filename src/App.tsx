...
const topUpSemester = ['MATH213 Transform and Boundary Values Problem — C (64)','BEEE223 Analog Communication Systems — B+ (79)','BEEE215 Electrical Wiring Design — B+ (78)','BEEE227 Programming With C++ — B (72)','BEEE217 Electromagnetic Fields Theory — A+ (88)','BEEE231 DC Machines — B+ (76)','BEEE243 Analog Communication and DC Machines Lab — B+ (77)','BEEE245 Engineering Economics — C+ (68)','BSTA247 Research Methodology for Engineers — B (72)'];
  ...
  return <div className="site-shell">
    <header className="site-header">
      ...
      <nav className={`main-nav ${menuOpen ? 'is-open' : ''}`}>
        <a href="#about" onClick={closeMenu}>About</a><a href="#work" onClick={closeMenu}>Selected work</a><a href="#journey" onClick={closeMenu}>Journey</a><a href="#academics" onClick={closeMenu}>Academics</a><a href="#contact" onClick={closeMenu}>Contact</a><a className="nav-cta" href="mailto:edwardwondermordey@gmail.com" onClick={closeMenu}>Let's talk <ArrowUpRight size={16}/></a>
      </nav>
    </header>
    ...
    <main id="top">
      ...
      <section className="education-section section-wrap" id="academics"><div className="section-label"><span>04</span><span>Academic profile</span></div><div className="education-grid"><div><h2>Study with<br/><em>purpose.</em></h2><p>A concise academic profile on the front page, with deeper transcript history available for anyone who wants to understand the journey behind the credentials.</p></div><div className="credential-list"><div className="credential-item"><GraduationCap size={25}/><div><span>Current · 4.0 / 4.0</span><h3>Bachelor of Applied Science in System Administration</h3><p>Ensign College · Final Year</p></div><ChevronDown size={18}/></div><div className="credential-item"><Award size={25}/><div><span>Completed · December 2025</span><h3>Diploma in Electrical & Electronic Engineering</h3><p>Kumasi Technical University · Final CGPA 3.84 · Second Class Upper Division</p></div><ChevronDown size={18}/></div><div className="credential-item"><Cpu size={25}/><div><span>Current · Top-up degree</span><h3>Bachelor of Engineering in Electrical & Electronics Engineering</h3><p>Kumasi Technical University · Level 2 · Current CGPA 3.6750</p></div><ChevronDown size={18}/></div><div className="credential-item"><Check size={25}/><div><span>Certificates</span><h3>IT Professional · Technical Support Engineer · PathwayConnect</h3><p>TestOut Network Pro · Hybrid Server Pro · Client Pro</p></div><ChevronDown size={18}/></div></div></div>
        <div className="transcript-block" id="diploma-transcript"><h3>Diploma transcript history</h3>{diplomaSemesters.map(([title,summary,courses])=><details key={title}><summary><strong>{title}</strong><span>{summary}</span></summary><ul>{courses.map(c=><li key={c}>{c}</li>)}</ul></details>)}</div><div className="transcript-block" id="topup-transcript"><h3>Current top-up semester record</h3><details open><summary><strong>Current coursework</strong><span>Selected results</span></summary><ul>{topUpSemester.map(c=><li key={c}>{c}</li>)}</ul></details></div>
      </section>

      <section className="contact-section section-wrap" id="contact"><div className="contact-panel"><div className="contact-copy"><p className="eyebrow">05 · Contact</p><h2>Let's build something<br/><em>worth documenting.</em></h2><p>Have a systems challenge, data problem, or thoughtful project in mind? I'd be glad to start a conversation.</p></div><div className="contact-actions"><a className="contact-link" href="mailto:edwardwondermordey@gmail.com"><Mail size={17}/> Email <ArrowUpRight size={16}/></a><a className="contact-link" href="https://github.com/Edward-Wonder-Mordey" target="_blank" rel="noreferrer"><Github size={17}/> GitHub <ArrowUpRight size={16}/></a><a className="contact-link" href="https://www.linkedin.com/in/edward-wonder-mordey/" target="_blank" rel="noreferrer"><Linkedin size={17}/> LinkedIn <ArrowUpRight size={16}/></a></div></div></section>
    </main>
